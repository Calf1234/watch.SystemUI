    ...
    private void refreshUsers(int forcePictureLoadForId) {
        ...
        new AsyncTask<SparseArray<Bitmap>, Void, ArrayList<UserRecord>>() {
            ...
            protected ArrayList<UserRecord> doInBackground(SparseArray<Bitmap>... params) {
                ...
                /* INTEL PRIVATE GARDEN begin */
                // if current is PG, then do not show any icons except for itself.
                // if current is not PG, then show any icons except for PG.
                if (UserManager.isPgCurrentUser()) {
                    records.clear();
                    UserInfo pgInfo = mUserManager.getUserInfo(currentId);
                    Bitmap picture2 = bitmaps.get(pgInfo.id);
                    if (picture2 == null) {
                        picture2 = mUserManager.getUserIcon(pgInfo.id);
                    }
                    if (picture2 != null) {
                        picture2 = BitmapHelper.createCircularClip(picture2, avatarSize, avatarSize);
                    }
                    records.add(0, new UserRecord(pgInfo, picture2, false /* isGuest */,
                            true, false /* isAddUser */, false /* isRestricted */));
                    return records;
                }
                /* INTEL PRIVATE GARDEN end */
                for (UserInfo info : infos) {
                    /* INTEL PRIVATE GARDEN begin */
                    // if user is pg user, then continue, do not show pg icon.
                    if(info.isPrivateGarden()){
                        continue;
                    }
                    /* INTEL PRIVATE GARDEN end */
