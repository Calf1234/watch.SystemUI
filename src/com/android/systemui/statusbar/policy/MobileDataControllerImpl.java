...
public class MobileDataControllerImpl implements NetworkController.MobileDataController {
    ...
    public DataUsageInfo getDataUsageInfo(int subId, String carrierName) {
        final String subscriberId = mUxFlag ? getActiveSubscriberId(mContext, subId)
                        : getActiveSubscriberId(mContext);
        ...
        try {
            ...
            if (usage != null) {
                usage.carrier = mUxFlag ? ((carrierName == null) ? mNetworkController.getMobileNetworkName() : carrierName)
                                : mNetworkController.getMobileNetworkName();
            }
            return usage;
        } catch (RemoteException e) {
    ...
    public void setMobileDataEnabled(int subId, boolean enabled, boolean refresh) {
        mTelephonyManager.setDataEnabled(subId, enabled);
        if (mCallback != null && refresh) {
            mCallback.onMobileDataEnabled(enabled);
        }
    }
    ...
    public boolean isMobileDataEnabled(int subId) {
