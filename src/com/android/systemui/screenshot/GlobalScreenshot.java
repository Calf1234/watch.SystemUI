...
class SaveImageInBackgroundTask extends AsyncTask<SaveImageInBackgroundData, Void,
    ...
    SaveImageInBackgroundTask(Context context, SaveImageInBackgroundData data,
        ...
        if (mUxFlag) {
            // Handle screenshot save path switch: built-in emulated OR external SD card ?
            ContentResolver resolver = context.getContentResolver();
            int saveToExt = Settings.System.getInt(resolver, "screenshot_to_ext", 0);
            String savePath = Settings.System.getString(resolver, "screenshot_ext_path");
            boolean extStorageReady = false;
            if (savePath != null) {
                String state = Environment.getStorageState(new File(savePath));
                extStorageReady = Environment.MEDIA_MOUNTED.equals(state);
            }
            if (saveToExt == 1 && extStorageReady) {
                File extPicDir = new File(savePath, "Pictures");
                mScreenshotDir = new File(extPicDir, SCREENSHOTS_DIR_NAME);
            } else {
                // If no accessible external SD card, fallback to built-in emulated storage
