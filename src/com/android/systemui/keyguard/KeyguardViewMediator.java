    ...
    KeyguardUpdateMonitorCallback mUpdateCallback = new KeyguardUpdateMonitorCallback() {
        ...
        public void onUserSwitching(int userId) {
            ...
            synchronized (KeyguardViewMediator.this) {
                ...
                if (DEBUG) Log.d(TAG, "onUserSwitching : userId=" + userId);
                if (UserManager.isPgUser(userId) ||
                    (userId == UserHandle.USER_OWNER && UserManager.switchingFromPgUser())) {
                    UserManager.get(mContext).setSwitchingOldUser(-1);
                    // make the keyguard not show when switch to a user
