...
public class AirplaneModeTile extends QSTile<QSTile.BooleanState> {
    ...
    private boolean isOpForAirplane = false;
    ...
    public void handleClick() {
        ...
        Log.d("AirplaneModeTile",  "handleClick mState:" + mState.clickable +", airplanmode:"+radioPowerMode);

        if(!radioPowerMode.equals("0")) {
            mState.clickable = false;
            Log.d("AirplaneModeTile", "handleClick - Ignore the click as last flight mode switching not finished");
        }
        else if(!isOpForAirplane){
          isOpForAirplane = true;
          setEnabled(!mState.value);
          mState.clickable = false;
          handleRefreshState(mState);
        }
    ...
    private void registerAirplanMode(){
        ...
        mContext.registerReceiver(mRadioModeChangeReceiver, filter);
    }

    private BroadcastReceiver mRadioModeChangeReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
           isOpForAirplane = false;
           mState.clickable = true;
           handleRefreshState(mState);
           Log.d("AirplaneModeTile",  "mRadioModeChangeReceiver onReceive mState:" + mState.clickable);
